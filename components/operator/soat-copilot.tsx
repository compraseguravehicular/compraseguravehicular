...
function imageFileName(file: File, source: ImageInputSource) {
  ...
  if (source === "paste") {
    return "soat-screenshot-pegado.png";
  }

  if (source === "drop") {
    return "soat-captura-arrastrada.png";
  }
...
export function SoatCopilot({
  ...
  const submitEvidence = useCallback(async (textToAnalyze = rawText) => {
    ...
    if (evidence.length < 20) {
      setState({
        status: "error",
        message: "Necesito la tabla, HTML o captura OCR del resultado SOAT."
      });
      return;
    }
    ...
    try {
      const response = await fetch("/api/operator/soat/parse", {
        method: "POST",
    ...
    try {
      ...
      if (extractedText.length < 20) {
        setOcrState({
          status: "error",
          fileName,
          message: "El OCR no pudo leer suficiente texto. Sube una captura mas nitida de la tabla SOAT."
        });
        return;
      }
  ...
  async function handleImageDrop(event: React.DragEvent<HTMLLabelElement>) {
    ...
    if (!imageFile) {
      setOcrState({
        status: "error",
        message: "Suelta una imagen valida de la tabla SOAT."
      });
      return;
    }
            ...
            <article className="min-w-0 rounded-md border border-line bg-white p-5 shadow-panel">
              ...
              {state.data.result.fieldRows.length ? (
                ...
                <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
                  ...
                  del resultado APESEG o sube una captura mas nitida.
                </div>
              )}
            </article>
                  ...
                  {state.data.result.alerts.length ? (
                    ...
                    <div className="flex gap-3 text-ink">
                      <CheckCircle2
                        ...
                        size={17}
                      />
                      <span>Sin alerta SOAT evidente en el texto capturado.</span>
                    </div>
                  )}
                </div>
              </div>
